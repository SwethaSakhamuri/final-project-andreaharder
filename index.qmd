...
```{rdata_import, echo=TRUE, message = FALSE, eval = TRUE}
library(tidyverse)
library(dplyr)
library(ggplot2)
library(leaflet)
...
#Plot 3: Case Information Fatalities overtime
...
#Plot 6: Case Information Lighting Conditions 
Lighting <- CaseInfo %>%
  mutate(lighting_conditions = factor(lighting_conditions, levels = c("Dawn", "Daylight", "Dusk", "Dark-Road Lighted", "Dark-Road Unlighted", "Unknown"),ordered = TRUE))%>%
  group_by(lighting_conditions)%>%
  count(lighting_conditions)
names(Lighting)[2] <- "Count"

ggplot(data = Lighting, aes(x= lighting_conditions, y= Count))+
  geom_bar(stat = "identity")+
